...
一个面向海外哔哩哔哩用户的多线程视频加速扩展。它会把视频分段拆成多个连续的字节区间并行下载，主要用于改善冷门视频和高码率四千分辨率视频的卡顿问题。
...
扩展使用“后台预热后接管”，避免一打开视频就出现长时间黑屏：

1. 哔哩哔哩原播放器先正常显示和播放；
2. 多线程播放器在后台读取音视频索引并下载首段；
3. 视频和音频都追上原播放器当前进度，并多出约两秒缓存；
4. 扩展同步播放时间、音量和倍速，然后显示新的播放器。

因此，打开视频几秒后播放器才发生变化是正常现象。如果所选节点一直比原播放器更慢，扩展可能暂时不接管，以免加速插件反而拖慢播放。
...
3. 把每个媒体分段按连续字节位置拆成多个小块；
4. 使用标准字节范围请求并行下载；
5. 核对每个响应的起止位置、长度和文件总大小；
6. 将全部小块按原位置重新组合；
7. 写入浏览器媒体缓冲区，由播放器继续播放。

只有全部字节块通过校验后，这一段才会交给播放器，避免错误内容造成花屏、解码失败或时间轴错乱。
